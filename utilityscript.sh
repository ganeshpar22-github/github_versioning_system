...
for BRANCH_REF in "${RELEASE_REFS[@]}"; do
    ...
    VERSION_PREFIX=$(echo "$BRANCH_NAME" | sed 's/release\/crew-//')
    echo "Derived VERSION_PREFIX: $VERSION_PREFIX"
    ...
    # once give version_prefix later on change it to VERSION_PREFIX
